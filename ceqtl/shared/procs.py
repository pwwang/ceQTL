...
class DataPreparation(Proc):
    ...
    Input:
        ...
        triofile: The trio file limits TF-snp-gene relationship.
    ...
    """
    input = "geno:file, expr:file, cov:file, genesnp:file, tftarget:file, triofile:file"
...
class DataPreparationTrio(Proc):
    """Prepare the data for the analysis.
    ...
    2. checks whether the number and order of samples in the covariate matrix and the
    expression matrix are the same.
    3. Use the trio file to limit the TF-snp-gene relationship.
    ...
    Envs:
        ...
        transpose_cov (flag): Whether to transpose the covariate matrix if
            the covariates are in rows.
    """
    input = "geno:file, expr:file, cov:file, triofile:file"
    output = "outdir:dir:{{in.geno | stem}}.prepared"
    lang = config.lang.rscript
    envs = {
        "transpose_geno": False,
    ...
    script = "file://scripts/DataPreparationTrio.R"
